...
<h4 class="modal-title"><b><span class="employee_id">Update Payment Details for: <br> <span style="color: green; font-weight:bold;">{{ $employees->name }} </span> CTG Number <span style="color: green; font-weight:bold;">{{$employees->ctgnumber }}</span></span></b></h4>
  </center>
            <div class="modal-body text-left">
              <div class="container-fluid">
                <form class="form-horizontal" method="POST" action="{{ route('payment.edit', $employees->id) }}">
                    @csrf
                    @method('PUT')

                                        
                    <input type="hidden" class="input-md textinput textInput form-control" id="name" name="name" value="{{ $employees->name }}"
                    required>
                    <input type="hidden" class="input-md textinput textInput form-control" id="ctgnumber" name="ctgnumber" value="{{ $employees->ctgnumber }}"
                    required>
                    <input type="hidden" class="input-md textinput textInput form-control" id="bsalary" step="1" name="bsalary" value="{{ $employees->bsalary }}"
                    required>
                  <div class="row">
                    {{-- <div id="div_id_name" class="form-group required col-md-4"> 
                      ...
                      {{-- <div class="controls col-md-12 "> 
                          <input type="number" class="input-md textinput textInput form-control" id="daysworked" name="daysworked" value="{{ $datas->attendance_time }}"
                           required>
                      </div> --}}
                      @foreach( $data as $key => $datas)                    
                      
                      <div class="controls col-md-12 "> 
                          <input type="hidden" class="input-md textinput textInput form-control" id="daysworked" name="daysworked" value=" {{ $datas->attendance_time }}"
                           required>
                      </div>
                      {{-- <div id="div_id_name" class="form-group required col-md-4"> 
    ...
    <center>
      <div class="form-group">
        ...
        <select class="form-control" id="batch" name="batch" required>
            <option value="" selected>- Select Payroll Batch Roll -</option>
